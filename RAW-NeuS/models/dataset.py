...
class Dataset:
    def __init__(self, conf):
        ...
        self.images_np = np.stack([nefread(im_name) for im_name in self.images_lis]) / 1.0 / 1.0  #RAW
        ...
        for scale_mat, world_mat in zip(self.scale_mats_np, self.world_mats_np):
            ...
            self.intrinsics_all.append(torch.from_numpy(intrinsics).float()) #将数组转化为张量并共享内存
    ...
    def image_at(self, idx, resolution_level):
        ...
        return (cv.resize(rgbimg, (self.W // resolution_level, self.H // resolution_level))).clip(0, 255)
